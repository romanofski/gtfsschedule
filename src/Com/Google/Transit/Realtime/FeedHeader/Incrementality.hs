...
module Com.Google.Transit.Realtime.FeedHeader.Incrementality (Incrementality(..)) where
import Prelude ((+), (/), (.))
...
instance Prelude'.Enum Incrementality where
  ...
  toEnum
   = P'.fromMaybe
      (Prelude'.error "hprotoc generated code: toEnum failure for type Com.Google.Transit.Realtime.FeedHeader.Incrementality")
      . toMaybe'Enum
  succ FULL_DATASET = DIFFERENTIAL
  succ _ = Prelude'.error "hprotoc generated code: succ failure for type Com.Google.Transit.Realtime.FeedHeader.Incrementality"
  pred DIFFERENTIAL = FULL_DATASET
  pred _ = Prelude'.error "hprotoc generated code: pred failure for type Com.Google.Transit.Realtime.FeedHeader.Incrementality"
...
instance P'.ReflectEnum Incrementality where
  ...
  reflectEnumInfo _
   = P'.EnumInfo
      (P'.makePNF (P'.pack ".transit_realtime.FeedHeader.Incrementality") [] ["Com", "Google", "Transit", "Realtime", "FeedHeader"]
        "Incrementality")
      ["Com", "Google", "Transit", "Realtime", "FeedHeader", "Incrementality.hs"]
