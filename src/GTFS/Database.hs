...
searchStopCode dbfile s =
  runDBWithoutLogging dbfile $
    do
      records <- searchStopCodeByName s
      return $
        ( \e ->
            StopSearchResult
...
searchStopCodeByName ::
  ...
  String ->
  ReaderT Sqlite.SqlBackend m [Sqlite.Entity Stop]
searchStopCodeByName str =
  select $
    from $
      \s ->
        do
          where_ (s ^. StopName `like` (%) ++. val str ++. (%))
          return s
...
getNextDepartures stopID now nowDate l = select $
  from $ \(st, t, c, s, r) -> do
    where_
    ...
    return (st, t, r, s)
  where
    earliest = timeToTimeOfDay $ timeOfDayToTime now - queryTimeWindowEarliest nextServicesTimeWindow
...
addDatabaseIndices = do
  ...
  where
    indices =
      [ "create index stop_time_index ON stop_time (stop_id);",
